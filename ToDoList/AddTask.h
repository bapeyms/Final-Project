...
void AddTask(Task* task)
{
	...
	while (task->time.hours < 0 || task->time.hours > 24 || task->time.minutes < 0 || task->time.minutes > 60)
	{
		if (task->time.hours < 0 || task->time.hours > 24)
		{
			std::cout << "Please, enter a value from 01 to 23 for the hour!" << std::endl;
			std::cout << "Execution time (HH MM): ";
			std::cin.clear();
			std::cin.ignore(10000, '\n');
			std::cin >> task->time.hours >> task->time.minutes;
		}
		else if (task->time.minutes < 0 || task->time.minutes > 60)
		{
			std::cout << "Please, enter a value from 01 to 59 for the minutes!" << std::endl;
			std::cout << "Execution time (HH MM): ";
			std::cin.clear();
			std::cin.ignore(10000, '\n');
			std::cin >> task->time.hours >> task->time.minutes;
